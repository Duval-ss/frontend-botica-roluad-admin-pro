...
export default function Checkout() {
  ...
  const buscarDocumento = async () => {
    ...
    if (!docLimpio) {
      return setError('Ingresa un DNI o RUC.')
    }

    if (!/^\d+$/.test(docLimpio)) {
      return setError('El documento solo debe contener números.')
    }

    if (docLimpio.length !== 8 && docLimpio.length !== 11) {
      return setError('El documento debe tener 8 dígitos para DNI o 11 para RUC.')
    }

    try {
      setConsultandoDoc(true)

      const { data } = await api.get(`/consulta/${docLimpio}`)
      ...
      if (docLimpio.length === 8) {
        ...
        setForm((prev) => ({
          ...prev,
          nombre_cliente: nombreCompleto || prev.nombre_cliente
        }))
      ...
      if (docLimpio.length === 11) {
        setForm((prev) => ({
          ...prev,
          nombre_cliente: data.razonSocial || prev.nombre_cliente,
      ...
      setError(err.response?.data?.message || 'No se pudo consultar el DNI/RUC.')
    } finally {
  ...
  const confirmarPedido = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        documento_cliente: documento.trim() || null,
        tipo_documento: documento.trim().length === 8 ? 'DNI' : documento.trim().length === 11 ? 'RUC' : null, // se agrego esto
          ...
          <div className="card border-0 shadow-sm rounded-4">
            <div className="card-body p-4">
              ...
              <p className="text-muted">
                Puedes comprar como invitado o iniciar sesión si deseas guardar tu historial.
              </p>
              ...
              <form onSubmit={confirmarPedido}>
                <div className="row g-3">
                  <div className="col-12">
                    ...
                    <div className="input-group">
                      ...
                      >
                        {consultandoDoc ? 'Buscando...' : 'Buscar'}
                      </button>
                    </div>
                  ...
                  <div className="col-12">
                    ...
                    <div className="delivery-options">
                      <button
                        type="button"
                        className={`delivery-option ${form.tipo_entrega === 'DELIVERY' ? 'active' : ''}`}
                        onClick={() => actualizar('tipo_entrega', 'DELIVERY')}
                      >
                        🚚 Delivery
                      </button>
                      ...
                      <button
                        type="button"
                        className={`delivery-option ${form.tipo_entrega === 'RECOJO' ? 'active' : ''}`}
                        onClick={() => actualizar('tipo_entrega', 'RECOJO')}
                      >
                        🏪 Recojo en botica
                      </button>
                    </div>
                ...
                <button
                  ...
                  disabled={enviando || consultandoDoc}
                >
